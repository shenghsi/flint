...
- [x] 1.5 Bundle a concise `flintctl` skill with task-based trigger metadata, a `FLINT_AGENT_THREAD=1` guard, release-channel marker discovery, and current thread and terminal command guidance; validate its structure and exact safety text.
...
- [x] 1.9 Set `FLINT_AGENT_THREAD=1` only for local Agent Threads and verify local, Direct remote, and Tunneled remote launch boundaries.
