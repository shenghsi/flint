...
- [x] 1.5 Bundle a concise `flintctl` skill with task-based trigger metadata, release-channel marker and endpoint discovery, an authoritative `terminal current --json` caller probe, and current thread and terminal command guidance; validate its structure and safety behavior.
- [x] 1.6 Add `flintctl skill print`, `status`, `install`, `update`, and `uninstall` with verified per-agent destinations, explicit ownership metadata, content digests, atomic replacement, and user-modification conflict handling; verify command and filesystem tests cover each lifecycle state.
- [x] 1.7 Stop launch-time writes to global instruction files and add launch-time updates only for recorded Flint-owned skills; leave existing managed global blocks unchanged and verify an uninstalled agent causes no instruction or skill file changes.
- [x] 1.9 Keep caller detection independent of terminal environment variables and verify local, Direct remote, and Tunneled remote launch boundaries.
- [x] 1.10 Add a Settings Editor action and Agent Threads UI flow that previews the bundled skill, selects supported agents, requires consent, reports conflicts, and offers uninstall; verify the action and UI capability gates.
...
- [x] 6.4 Verify each application launch rewrites the bundled marker to the packaged `flintctl`, updates only recorded unchanged Flint-owned skills, and does not write general instruction files; run the focused marker, skill lifecycle, package, and route tests.
...
- [ ] 7.3 Run the focused test suites for `agent_control_protocol`, `agent_control_cli`, `agent_threads`, `terminal`, `terminal_view`, `workspace`, `util`, `settings_ui`, and `auto_update_helper`, and record any platform-only test coverage in the pull request.
- [x] 7.4 Build the local macOS bundle with `./script/bundle-tmp-app`; if the documented debug `remote_server` signing step fails, copy the fresh debug bundle manually, then verify `/tmp/Flint-Local.app` contains `flintctl`, contains the release-matched skill, and does not contain `flint-agent-control`.
