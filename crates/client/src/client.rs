...
actions!(
    client,
    [
        /// Signs in to Flint account.
        SignIn,
        /// Signs out of Flint account.
        SignOut,
...
pub struct ClientSettings {
    ...
    ///
    /// Useful when running multiple Flint instances side by side without them
...
impl ClientCredentialsProvider {
    pub fn new(cx: &App) -> Self {
        Self {
            provider: flint_credentials_provider::global(cx),
        }
    }
...
impl Client {
    ...
    ///
    /// Only Flint staff automatically connect to Collab.
    ...
    fn establish_websocket_connection(
        ...
        cx.spawn(async move |cx| {
            ...
            request_headers.insert(
                "x-flint-protocol-version",
                HeaderValue::from_str(&rpc::PROTOCOL_VERSION.to_string())?,
            );
            request_headers.insert("x-flint-app-version", HeaderValue::from_str(&app_version)?);
            request_headers.insert(
                "x-flint-release-channel",
            ...
            if let Some(system_id) = system_id {
                request_headers.insert("x-flint-system-id", HeaderValue::from_str(&system_id)?);
            }
            if let Some(metrics_id) = metrics_id {
                request_headers.insert("x-flint-metrics-id", HeaderValue::from_str(&metrics_id)?);
            }
        ...
        cx.spawn(async move |cx| {
            ...
            let credentials = background
                ...
                .spawn(async move {
                    ...
                    // flint server to encrypt the user's access token, so that it can'be intercepted by
                    ...
                    // Start an HTTP server to receive the redirect from Flint's sign-in page.
                    ...
                    // Open the Flint sign-in page in the user's browser, with query parameters that indicate
                    // that the user is signing in from a Flint app running on the same device.
    ...
    async fn authenticate_as_admin(
        ...
        let url = self
            .http
            .build_flint_cloud_url("/internal/users/impersonate")?;
...
/// prefix for the flint:// url scheme
pub const ZED_URL_SCHEME: &str = "flint";
...
/// Parses the given link into a Flint link.
///
/// Returns a [`Some`] containing the parsed link if the link is a recognized Flint link
/// that should be handled internally by the application.
/// Returns [`None`] for links that should be opened in the browser.
pub fn parse_flint_link(link: &str, cx: &App) -> Option<FlintLink> {
    ...
    let Some(next) = parts.next() else {
        return Some(FlintLink::Channel { channel_id });
    };

    if let Some(heading) = next.strip_prefix("notes#") {
        return Some(FlintLink::ChannelNotes {
            channel_id,
