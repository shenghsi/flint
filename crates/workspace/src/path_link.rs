        ...
        BackgroundPathChecks::LocalFileSystem => {
            ...
            cx.background_spawn(async move {
                for mut path_to_check in paths_to_check {
                    if let Some(resolved_path) = fs.canonicalize(&path_to_check.path).await.ok()
                        && let Some(metadata) = fs.metadata(&resolved_path).await.ok().flatten()
                    {
                        if open_target
                            .as_ref()
                            .map(|target| target.path().path != resolved_path)
                            .unwrap_or(true)
                        {
            ...
            cx.spawn(async move |cx| {
                for mut path_to_check in paths_to_check {
                    ...
                    if let Some(resolved_path) = resolved_path {
                        ...
                        if let Some(resolved_path) = resolved_path.into_abs_path() {
        ...
        .chain(potential_paths.iter().flat_map(|path_to_check| {
            ...
            if maybe_path.starts_with("~") {
                if let Some(home_path) = maybe_path
                    .strip_prefix("~")
                    .ok()
                    .and_then(|stripped_path| Some(dirs::home_dir()?.join(stripped_path)))
                {
