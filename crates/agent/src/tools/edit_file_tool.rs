...
mod tests {
    ...
    async fn test_streaming_authorize(cx: &mut TestAppContext) {
        ...
        assert_eq!(
            ...
            Some("Edit `.flint/settings.json` (local settings)".into())
        );
        ...
        // Test 3: Relative path without .flint should not require confirmation
        let (stream_tx, mut stream_rx) = ToolCallEventStream::test();
        ...
        // Test 4: Path with .flint in the middle should require confirmation
        let (stream_tx, mut stream_rx) = ToolCallEventStream::test();
        let _auth = cx.update(|cx| {
            edit_tool.authorize(&PathBuf::from("root/.flint/tasks.json"), &stream_tx, cx)
        });
        let event = stream_rx.expect_authorization().await;
        assert_eq!(
            event.tool_call.fields.title,
            Some("Edit `root/.flint/tasks.json` (local settings)".into())
        );
    ...
    /// `.flint/foo/../../safe.json` similarly sidesteps the consecutive-
    /// component scan for `.flint/`, so the canonical-path recheck has to
    /// catch it. (We escape *out* of `.flint/` here and back in via `..`,
    ...
    async fn test_streaming_authorize_blocks_dotdot_settings_bypass(cx: &mut TestAppContext) {
        ...
        let _auth = cx.update(|cx| {
            edit_tool.authorize(
                &PathBuf::from(path!("/root/.flint/foo/../settings.json")),
                &stream_tx,
                cx,
            )
        });
        ...
        assert!(
            ...
            "`..` traversal into .flint must still prompt: {:?}",
            event.tool_call.fields.title,
        );
    }

    /// An intra-project symlink like `safe -> .flint` keeps a path's
    /// raw components clean of `.flint`, and `resolve_project_path`
    ...
    async fn test_streaming_authorize_blocks_intra_project_symlink_bypass(cx: &mut TestAppContext) {
        ...
        .await;
        fs.insert_symlink(path!("/root/safe"), PathBuf::from(".flint"))
            .await;
        let (edit_tool, _project, _action_log, _fs, _thread) =
            setup_test_with_fs(cx, fs, &[path!("/root").as_ref()]).await;
        ...
        assert!(
            event
                ...
                .as_deref()
                .is_some_and(|title| title.ends_with("(local settings)")),
            "Intra-project symlink to .flint must still prompt: {:?}",
            event.tool_call.fields.title,
        );
    }
    ...
    async fn test_streaming_needs_confirmation_with_multiple_worktrees(cx: &mut TestAppContext) {
        ...
        let test_cases = vec![
            ...
            (
                "shared/.flint/settings.json",
                true,
                ".flint file in third worktree",
            ),
