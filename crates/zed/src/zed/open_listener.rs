    ...
    pub fn parse(request: RawOpenRequest, cx: &App) -> Result<Self> {
        ...
        for url in request.urls {
            if let Some(server_name) = url.strip_prefix("zed-cli://") {
                ...
                || url.starts_with("zed://agent-server")
            {
            ...
            } else if client::parse_zed_link(&url, cx).is_some() {
...
mod tests {
    ...
    fn test_retired_agent_urls_are_ignored(cx: &mut TestAppContext) {
        ...
        for url in [
            ...
            "zed://agent/shared/123e4567-e89b-12d3-a456-426614174000",
            "zed://skill?data=unused",
        ] {
            let request = cx.update(|cx| {
                OpenRequest::parse(
                    RawOpenRequest {
