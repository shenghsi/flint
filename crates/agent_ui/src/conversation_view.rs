...
use flint_actions::agent::{Chat, ToggleModelSelector};
...
impl ConversationView {
    ...
    fn spawn_external_agent_login(
        ...
        window.spawn(cx, async move |cx| {
            ...
            if let Some(cmd) = &task.command {
                // Have "node" command use Flint's managed Node runtime by default
                if cmd == "node" {
    ...
    fn render_unsupported(
        ...
        let (heading_label, description_label) = (
            format!("Upgrade {} to work with Flint", self.agent.agent_id()),
            if version.is_empty() {
                format!(
