                        ...
                        completion_item: Some(CompletionItemCapability {
                            ...
                            resolve_support: Some(CompletionItemCapabilityResolveSupport {
                                properties: vec![
                                    ...
                                    // NB: Do not have this resolved, otherwise Flint becomes slow to complete things
                                    // "textEdit".to_string(),
                                ],
                            }),
    ...
    fn drop(&mut self) {
        match self {
            Subscription::Notification {
                ...
                if let Some(handlers) = notification_handlers.as_ref().and_then(Weak::upgrade) {
...
mod tests {
    ...
    #[gpui::test]
    async fn test_retained_subscription_does_not_keep_dropped_server_handlers_alive(
        ...
        server
            .on_notification::<notification::ShowMessage, _>(move |_, _| {
                let _detached_payload = &detached_payload;
            })
            .detach();
        ...
        let subscription =
            server.on_notification::<notification::PublishDiagnostics, _>(move |_, _| {
                let _retained_payload = &retained_payload;
            });
        ...
        let server = cx
            ...
            .await
            .expect("language server should initialize");
        ...
        assert!(
            ...
            "a retained subscription kept an unrelated detached handler alive"
        );
        assert!(
            retained_payload_handle.upgrade().is_none(),
            "a retained subscription kept its handler alive after server shutdown"
        );
