            ...
            0 => {
                workspace
                    .update_in(cx, |workspace, window, cx| {
                        workspace
                            .toggle_modal(window, cx, |_window, cx| InstallingFlintModal::new(cx));
                    })
                    .ok();
                if let Err(error) = move_to_applications(&self.app_path, cx).await {
                    workspace
                        .update_in(cx, |workspace, _window, cx| {
                            if let Some(modal) = workspace.active_modal::<InstallingFlintModal>(cx)
                            {
...
pub struct InstallingFlintModal {
    focus_handle: FocusHandle,
    finished: bool,
}

impl InstallingFlintModal {
...
impl EventEmitter<DismissEvent> for InstallingFlintModal {}

impl ModalView for InstallingFlintModal {
...
impl Focusable for InstallingFlintModal {
    fn focus_handle(&self, _cx: &App) -> FocusHandle {
        self.focus_handle.clone()
    }
}

impl Render for InstallingFlintModal {
