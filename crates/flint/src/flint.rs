...
pub fn init(cx: &mut App) {
    ...
    .on_action(|_: &flint_actions::OpenLicenses, cx| {
        with_active_or_new_workspace(cx, |workspace, window, cx| {
            open_bundled_file(
                workspace,
    ...
    .on_action(|&flint_actions::OpenKeymapFile, cx| {
    ...
    .on_action(|_: &flint_actions::OpenDefaultKeymap, cx| {
        with_active_or_new_workspace(cx, |workspace, window, cx| {
            open_bundled_file(
                workspace,
...
fn register_actions(
    ...
    workspace
        ...
        .register_action(|workspace, action: &flint_actions::OpenRemote, window, cx| {
        ...
        .register_action({
            ...
            move |_, action: &flint_actions::IncreaseUiFontSize, _window, cx| {
                if action.persist {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &flint_actions::DecreaseUiFontSize, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, cx| {
                        let ui_font_size = ThemeSettings::get_global(cx).ui_font_size(cx) - px(1.0);
                        let _ = settings
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &flint_actions::ResetUiFontSize, _window, cx| {
                if action.persist {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &flint_actions::IncreaseBufferFontSize, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, cx| {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &flint_actions::DecreaseBufferFontSize, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, cx| {
                        let buffer_font_size =
                            ThemeSettings::get_global(cx).buffer_font_size(cx) - px(1.0);
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &flint_actions::ResetBufferFontSize, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, _| {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &flint_actions::ResetAllZoom, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, _| {
                        settings.theme.ui_font_size = None;
                        settings.theme.buffer_font_size = None;
        ...
        .register_action(|_, _: &install_cli::RegisterFlintScheme, window, cx| {
            cx.spawn_in(window, async move |workspace, cx| {
                install_cli::register_flint_scheme(cx).await?;
                workspace.update_in(cx, |workspace, _, cx| {
                    struct RegisterFlintScheme;

                    workspace.show_toast(
                        Toast::new(
                            NotificationId::unique::<RegisterFlintScheme>(),
                            format!(
                                "flint:// links will now open in {}.",
            ...
            .detach_and_prompt_err(
                "Error registering flint:// scheme",
                window,
                cx,
                |_, _, _| None,
            );
        ...
        .register_action(
            |workspace: &mut Workspace,
             _: &flint_actions::project_panel::ToggleFocus,
             window: &mut Window,
             cx: &mut Context<Workspace>| {
...
mod tests {
    ...
    async fn test_opening_project_settings_when_excluded(cx: &mut gpui::TestAppContext) {
        ...
        eprintln!("Created project with .flint/settings.json containing UNIQUEVALUE");
        ...
        let original_settings = app_state
            .fs
            .load(Path::new("/root/.flint/settings.json"))
            .await
            .unwrap();
        ...
        // 3. Add .flint to file scan exclusions in user settings
        cx.update_global::<SettingsStore, _>(|store, cx| {
            store.update_user_settings(cx, |worktree_settings| {
                worktree_settings.project.worktree.file_scan_exclusions =
                    Some(vec![".flint".to_string()]);
            });
        });

        eprintln!("Added .flint to file_scan_exclusions in settings");
        ...
        // If .flint is not excluded, the test will fail here
        assert!(
            !has_flint_entry,
            "Test precondition failed: .flint directory should be excluded but was found in worktree"
        );
