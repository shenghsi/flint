...
/// Minimum Flint version for filtering captured examples.
/// For example, `MinCaptureVersion { major: 0, minor: 224, patch: 1 }` means only pull
/// examples where `flint_version >= 0.224.1`. The `major` component is required because Flint
    ...
    for (explicit, after_date) in after_timestamps.iter() {
        ...
        let statement = indoc! {r#"
            SELECT
                ...
                flint_version AS flint_version
            ...
            WHERE ep_outcome LIKE ?
                ...
                AND (? IS NULL OR (
                    COALESCE(TRY_CAST(SPLIT_PART(flint_version, '.', 1) AS INTEGER), 0) * 1000000
                    + COALESCE(TRY_CAST(SPLIT_PART(flint_version, '.', 2) AS INTEGER), 0) * 1000
                    + COALESCE(TRY_CAST(SPLIT_PART(SPLIT_PART(flint_version, '.', 3), '+', 1) AS INTEGER), 0)
                ) >= ?)
...
pub async fn fetch_requested_examples_after(
    ...
    for after_date in after_timestamps.iter() {
        ...
        let examples = fetch_examples_with_query(
            ...
            &["request_id", "device_id", "time", "input", "flint_version"],
...
fn rated_examples_from_response<'a>(
    ...
    let iter = response
        ...
        .filter_map(move |(row_index, data_row)| {
            ...
            let flint_version = get_string("flint_version");
...
fn build_rated_example(
    ...
    let mut example =
        build_example_from_snowflake(request_id, device_id, time, input, tags, None, flint_version);
