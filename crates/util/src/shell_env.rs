...
/// non-zero exit status even though `flint --printenv` ran successfully and
...
async fn capture_unix(
    ...
    let quoted_flint_path = super::get_shell_safe_flint_path(shell_kind)?;
    ...
    // See: https://github.com/zed-industries/flint/pull/32136#issuecomment-2999645482
    ...
    command_string.push_str(&format!("{} --printenv {}", quoted_flint_path, redir));
...
async fn capture_windows(
    ...
    let flint_path_string = flint_path.display().to_string();
    ...
    let quoted_flint_path = quote_for_shell(&flint_path_string)?;
    let cmd = match shell_kind {
        ...
        | ShellKind::Posix => cmd.args([
            ...
            "-i",
            "-c",
            &format!("cd {}; {} --printenv", quoted_directory, quoted_flint_path),
        ]),
        ShellKind::PowerShell | ShellKind::Pwsh => cmd.args([
            ...
            &format!(
                ...
                quoted_directory, quoted_flint_path
            ),
        ]),
        ...
        ShellKind::Nushell => {
            let flint_command = shell_kind
                .prepend_command_prefix(&quoted_flint_path)
                .into_owned();
            cmd.args([
                "-c",
                &format!("cd {}; {} --printenv", quoted_directory, flint_command),
            ])
        }
        ShellKind::Cmd => {
