...
const PRIVACY_DOCS: &str = "https://flint.dev/docs/ai/privacy-and-security";
...
impl Render for EditPredictionButton {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        match language_settings.edit_predictions.provider {
            ...
            provider @ (EditPredictionProvider::Flint | EditPredictionProvider::Mercury) => {
                let enabled = self.editor_enabled.unwrap_or(true);
                let file = self.file.clone();
                let language = self.language.clone();
                let project = self.project.clone();
                let provider_name: &'static str = match provider {
                    EditPredictionProvider::Flint => "flint",
                    _ => "unknown",
                };
                let icons = self
                ...
                if edit_prediction::should_show_upsell_modal(cx) {
                    ...
                    return div().child(
                        IconButton::new("flint-predict-pending-button", ep_icon)
                            .shape(IconButtonShape::Square)
                ...
                let flint_cloud_needs_sign_in =
                    matches!(provider, EditPredictionProvider::Flint) && user.is_none();
                let provider_unavailable =
                    missing_token || mercury_has_error || flint_cloud_needs_sign_in;

                let icon_button = IconButton::new("flint-predict-pending-button", ep_icon)
                    .shape(IconButtonShape::Square)
                    ...
                    .when(!self.popover_menu_handle.is_deployed(), |element| {
                        element.tooltip(move |_window, cx| {
                            ...
                            } else if flint_cloud_needs_sign_in {
...
impl EditPredictionButton {
    ...
    fn add_provider_switching_section(
        ...
        let is_flint_provider_disabled = organization_configuration
        ...
        if !providers.is_empty() {
            ...
            for provider in providers {
                ...
                let is_disabled_flint_provider =
                    provider == EditPredictionProvider::Flint && is_flint_provider_disabled;
    ...
    pub fn build_language_settings_menu(
        ...
        if matches!(provider, EditPredictionProvider::Flint) {
                                ...
                                v_flex()
                                    ...
                                    .child(
                                        Label::new(indoc!{
                                            ...
                                            Flint must detect a license file in your repo for this setting to take effect. \
        ...
        menu = menu.item(
            ContextMenuEntry::new("Configure Excluded Files")
                ...
                .documentation_aside(DocumentationSide::Left, |_| {
                    Label::new(indoc!{"
                        Open your settings to add sensitive paths for which Flint will never predict edits."}).into_any_element()
                })
                .handler(move |window, cx| {
                    telemetry::event!(
                        "Edit Prediction Menu Action",
    ...
    fn build_edit_prediction_context_menu(
        ...
        ContextMenu::build(window, cx, |mut menu, window, cx| {
            ...
            let needs_sign_in = user.is_none()
                && matches!(
                    provider,
                    EditPredictionProvider::None | EditPredictionProvider::Flint
                );
            ...
            if needs_sign_in {
                menu = menu
                    ...
                    .link_with_handler(
                        "Learn More",
                        OpenBrowser {
                            url: flint_urls::edit_prediction_docs(cx),
                        }
                        .boxed_clone(),
                        |_window, _cx| {
                            telemetry::event!(
                    ...
                    .and_then(|provider| provider.usage(cx))
                {
                    ...
                    menu = menu
                        .custom_entry(
                            ...
                            move |_, cx| cx.open_url(&flint_urls::account_url(cx)),
                        )
                    ...
                    menu = menu
                        ...
                        .entry(
                            "Check your payment status or contact us at billing-support@flint.dev to continue using this feature.",
                            None,
                            |_window, cx| {
                                cx.open_url(&flint_urls::account_url(cx))
                            },
...
pub fn get_available_providers(cx: &mut App) -> Vec<EditPredictionProvider> {
    ...
    providers.push(EditPredictionProvider::Flint);
