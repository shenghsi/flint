...
use client::{Client, TelemetrySettings, UserStore, flint_urls};
    ...
    v_flex()
        ...
        .child(
            SwitchField::new(
                ...
                None::<&str>,
                Some("Help improve Flint by sending anonymous usage data".into()),
        ...
        .child(
            SwitchField::new(
                ...
                None::<&str>,
                Some(
                    "Help fix Flint by sending crash reports so we can fix critical issues fast"
                        .into(),
                ),
...
fn render_worktree_auto_trust_switch(tab_index: &mut isize, cx: &mut App) -> impl IntoElement {
    ...
    SwitchField::new(
        ...
        Some("Automatically mark all new projects as trusted to unlock all Flint's features".into()),
        toggle_state,
        {
            let fs = <dyn Fs>::global(cx);
            move |&selection, _, cx| {
...
fn render_flint_agent_button(user_store: &Entity<UserStore>, cx: &mut App) -> impl IntoElement {
    let client = Client::global(cx);
    let status = *client.status().borrow();

    let plan = user_store.read(cx).plan();
    let is_free = matches!(plan, Some(Plan::FlintFree) | None);
    let is_pro = matches!(plan, Some(Plan::FlintPro));
    let is_trial = matches!(plan, Some(Plan::FlintProTrial));
    ...
    AgentSetupButton::new("flint-agent-onboarding")
        .icon(
            Icon::new(IconName::FlintAgent)
                .size(IconSize::XSmall)
                .color(Color::Muted),
        )
        .name("Flint Agent")
        ...
        .map(|this| {
            if is_signed_in && is_free {
                ...
                this.on_click(move |_, _, cx| {
                    telemetry::event!("Welcome Flint Agent Sign In Clicked");
                    let client = Client::global(cx);
...
fn render_ai_section(user_store: &Entity<UserStore>, cx: &mut App) -> impl IntoElement {
    ...
    let grid = FEATURED_AGENT_IDS.iter().fold(
        div()
            ...
            .gap_2()
            .child(render_flint_agent_button(user_store, cx)),
        |grid, agent_id| {
