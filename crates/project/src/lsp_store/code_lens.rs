...
impl LspStore {
    ...
    ///
    /// All visibility / batching policy lives in the caller. Remote resolves
    /// are forwarded to the host via [`Self::resolve_code_action`].
    pub fn resolve_code_lens(
        ...
        if self.upstream_client().is_some() {
            ...
            let task = cx
                .spawn(async move |lsp_store, cx| {
                    let response = resolve.await.context("resolving remote code lens");
                    lsp_store
                        .update(cx, |lsp_store, _| {
                            ...
                            let resolved = match response {
                                ...
                                Err(error) => {
                                    log::warn!("Failed to resolve remote code lens: {error:#}");
                                    return None;
                                }
                            };
