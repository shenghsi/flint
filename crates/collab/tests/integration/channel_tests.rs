...
async fn test_channel_room(
    ...
    active_call_a
        .update(cx_a, |active_call, cx| active_call.join_channel(flint_id, cx))
        .await
        .unwrap();
    ...
    cx_a.read(|cx| {
        client_a.channel_store().read_with(cx, |channels, _| {
            assert_participants_eq(
                channels.channel_participants(flint_id),
                &[client_a.user_id().unwrap()],
            );
        })
    });
    ...
    active_call_b
        .update(cx_b, |active_call, cx| active_call.join_channel(flint_id, cx))
        .await
        .unwrap();
    ...
    cx_a.read(|cx| {
        client_a.channel_store().read_with(cx, |channels, _| {
            assert_participants_eq(channels.channel_participants(flint_id), &[]);
        })
    });
...
async fn test_channel_jumping(executor: BackgroundExecutor, cx_a: &mut TestAppContext) {
    ...
    let flint_id = server
        .make_channel("flint", None, (&client_a, cx_a), &mut [])
        .await;
    let rust_id = server
...
async fn test_channel_link_notifications(
    ...
    let channels = server
        .make_channel_tree(&[("flint", None)], (&client_a, cx_a))
        .await;
    let flint_channel = channels[0];

    try_join_all(client_a.channel_store().update(cx_a, |channel_store, cx| {
        [
            channel_store.set_channel_visibility(flint_channel, proto::ChannelVisibility::Public, cx),
            channel_store.invite_member(flint_channel, user_b, proto::ChannelRole::Member, cx),
            channel_store.invite_member(flint_channel, user_c, proto::ChannelRole::Guest, cx),
        ]
    }))
    .await
    .unwrap();
    ...
    client_b
        .channel_store()
        .update(cx_b, |channel_store, cx| {
            channel_store.respond_to_channel_invite(flint_channel, true, cx)
        })
        .await
        .unwrap();
    ...
    // we have an admin (a), member (b) and guest (c) all part of the flint channel.
    ...
    let active_channel = client_a
        .channel_store()
        .update(cx_a, |channel_store, cx| {
            channel_store.create_channel("active", Some(flint_channel), cx)
        })
        .await
        .unwrap();
    ...
    assert_channels_list_shape(
        client_a.channel_store(),
        cx_a,
        &[(flint_channel, 0), (active_channel, 1)],
    );
    assert_channels_list_shape(
        client_b.channel_store(),
        cx_b,
    ...
    assert_channels_list_shape(client_c.channel_store(), cx_c, &[(flint_channel, 0)]);

    let vim_channel = client_a
        .channel_store()
        .update(cx_a, |channel_store, cx| {
            channel_store.create_channel("vim", Some(flint_channel), cx)
        })
        .await
        .unwrap();
    ...
    assert_channels_list_shape(
        client_a.channel_store(),
        cx_a,
        &[(flint_channel, 0), (active_channel, 1), (vim_channel, 1)],
    );
    assert_channels_list_shape(
        client_b.channel_store(),
        cx_b,
    ...
    assert_channels_list_shape(
        ...
        cx_c,
        &[(flint_channel, 0), (vim_channel, 1)],
    );

    let helix_channel = client_a
        .channel_store()
        .update(cx_a, |channel_store, cx| {
            channel_store.create_channel("helix", Some(flint_channel), cx)
        })
        .await
        .unwrap();
    ...
    assert_channels_list_shape(
        client_c.channel_store(),
        cx_c,
        &[(flint_channel, 0), (vim_channel, 1), (helix_channel, 2)],
    );
}
...
async fn test_channel_membership_notifications(
    ...
    let channels = server
        .make_channel_tree(
            &[("flint", None), ("vim", Some("flint")), ("opensource", None)],
            (&client_a, cx_a),
        )
        .await;
    let flint_channel = channels[0];
    ...
    try_join_all(client_a.channel_store().update(cx_a, |channel_store, cx| {
        [
            channel_store.set_channel_visibility(flint_channel, proto::ChannelVisibility::Public, cx),
            channel_store.set_channel_visibility(vim_channel, proto::ChannelVisibility::Public, cx),
            channel_store.invite_member(flint_channel, user_b, proto::ChannelRole::Admin, cx),
    ...
    // we have an admin (a), and a guest (b) with access to all of flint, and membership in vim.
    assert_channels(
        client_b.channel_store(),
        cx_b,
        &[
    ...
    client_b.channel_store().update(cx_b, |channel_store, _| {
        channel_store.is_channel_admin(flint_channel)
    });
