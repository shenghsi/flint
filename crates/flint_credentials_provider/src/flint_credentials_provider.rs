...
///
/// By default, running Flint in development uses the development credentials
...
pub struct FlintCredentialsProvider(pub Arc<dyn CredentialsProvider>);

impl Global for FlintCredentialsProvider {}
...
pub fn init_global(cx: &mut App) {
    ...
    let provider = new(cx);
    cx.set_global(FlintCredentialsProvider(provider));
}

pub fn global(cx: &App) -> Arc<dyn CredentialsProvider> {
    cx.try_global::<FlintCredentialsProvider>()
...
/// re-allow access to the system keychain when developing Flint.
