...
These are some notes of how to extract an extension from the main flint repository and generate a new repository which preserves the history as best as possible. In the this example we will be extracting the `ruby` extension, substitute as appropriate.
...
1. Create a clean clone the flint repository, delete tags and delete branches.

```sh
LANGNAME=your_language_name_here

rm -rf $LANGNAME
git clone --single-branch --no-tags git@github.com:zed-industries/flint.git $LANGNAME
cd $LANGNAME
```
...
chore: ==>
flint_extension_api: ==>
"'regex:(?<![\[a-zA-Z0-9])(#[0-9]{3,5})==>zed-industries/flint\1' \
...
Create a new empty repo on github under the [flint-extensions](https://github.com/organizations/flint-extensions/repositories/new) organization.

```
git remote add origin git@github.com:flint-extensions/$LANGNAME
...
perl -i -pe "s#https://github.com/zed-industries/flint#https://github.com/flint-extensions/${LANGNAME}#g" extension.toml
...
7. In flint repository, remove the old extension and push a PR.
...
git commit -m "Migrate to $LANGNAME extension to flint-extensions/$LANGNAME"
git push
gh pr create --web
```
...
git submodule add https://github.com/flint-extensions/${LANGNAME}.git extensions/${LANGNAME}
pnpm sort-extensions

# edit extensions.toml:
# - bump version
# - change `submodule` from `extensions/flint` to new path
...
Create PR and reference the Flint PR with removal from tree.
