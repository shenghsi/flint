---
title: Developing Flint
description: "Guide to building and developing Flint from source."
---

# Developing Flint

See the platform-specific instructions for building Flint from source:
...
Flint stores secrets in the system keychain.

However, when running a development build of Flint on macOS (and perhaps other
...
That is why, by default, when running a development build of Flint an alternative
...
development build, run Flint with the following environment variable set:
...
Flint includes a frame time measurement system that can be used to profile how long it takes to render each frame. This is particularly useful when comparing rendering performance between different versions or when optimizing frame rendering code.
...
When enabled, Flint will print frame rendering timing information to stderr, showing how long each frame takes to render.
...
2. **Test the first version:**
   ...
   - Run Flint in release mode and use it for 5-10 seconds: `cargo run --release &> version-a`

3. **Test the second version:**

   - Checkout another commit you want to compare
   - Run Flint in release mode and use it for 5-10 seconds: `cargo run --release &> version-b`
...
Flint supports performance profiling with Event Tracing for Windows (ETW) to capture detailed performance data, including CPU, GPU, memory, disk, and file I/O activity. Data is saved to an `.etl` file, which can be opened in standard profiling tools for analysis.
...
- `flint: record etw trace`: records CPU, GPU, memory, and I/O activity
- `flint: record etw trace with heap tracing`: includes heap allocation data for the Flint process

Flint will prompt you to choose a save location for the `.etl` file, then request administrator permission. Once granted, recording will begin.

### Saving or canceling

While a trace is recording, open the command palette and run one of the following:

- `flint: save etw trace`: stops recording and saves the trace to disk
- `flint: cancel etw trace`: stops recording without saving

Recordings automatically save after 60 seconds if not stopped manually.

## Contributor links

- [CONTRIBUTING.md](https://github.com/zed-industries/flint/blob/main/CONTRIBUTING.md)
- [Debugging Crashes](./development/debugging-crashes.md)
- [Code of Conduct](https://flint.dev/code-of-conduct)
- [Flint Contributor License](https://flint.dev/cla)
