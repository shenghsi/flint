...
| P1 | [#46541](https://github.com/zed-industries/zed/pull/46541) | `c31b2b0dc7180247b2981eb084594efaf11ee396` | v1.11 notes | Absent | `crates/git_ui`, `crates/project` | `reimplement` | #59884, Wave 1 complete | Tasks 5.2-5.3 multibuffers and hunk actions | `implementing` | [#109](https://github.com/shenghsi/flint/pull/109) | Task 5.2 landed separate staged HEAD-to-index and unstaged index-to-worktree views using Flint's existing `ProjectDiff` and `GitStore` architecture. Cached staged diffs refresh without duplicating worktree computation, retain stable buffer anchors, and avoid a `GitStore`/buffer lifetime cycle. Panel and entry actions reuse each exact view and switch repositories correctly. The reimplementation excludes upstream collaboration and broad buffer-diff refactors; Task 5.3 hunk actions remain. Deterministic Git UI execution passed 119 tests, including partial staging, deletion, rename, refresh, selection preservation, view reuse, and repository switching; formatting, clippy, and a signed local bundle also passed. |
