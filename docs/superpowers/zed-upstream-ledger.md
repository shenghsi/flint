...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| P0 | [#60584](https://github.com/zed-industries/zed/pull/60584) | `eb962794a33cbd414c6a343a8bb1136c94fe6901` | v1.13.0-pre notes | Absent | `crates/buffer_diff`, `crates/git_ui` | `adapt` | None | Task 1.1 repeated-line stage/unstage and index integrity | `proposed` | — | Canonicalize ambiguous hunk placement. |
...
| P0 | [#58275](https://github.com/zed-industries/zed/pull/58275) | `29622911de00305340012f798f17c04a274f4b31` | v1.8 notes | Absent | Removed `agent_ui` archival flow | `none` | None | Task 1.3 source and deletion-call-site evidence | `excluded` | — | Flint has no thread archival path that deletes worktrees. |
...
| Priority | Upstream PR | Final commit | Provenance | Fork ancestry | Flint paths | Strategy | Prerequisites | Required tests | Status | Flint PR | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| P0 | [#58994](https://github.com/zed-industries/zed/pull/58994) | `e569b955f55b1a2cee026a084eb16578c6e2e30a` | v1.6 notes | Absent | `crates/fs`, workspace dependency | `adapt` | Resolve final `notify` stack | Task 2.1 watcher registration completion | `proposed` | — | Review together with #59045, #59714, and #59560. |
| P0 | [#59045](https://github.com/zed-industries/zed/pull/59045) | `bae8065e2fbc0331cdd2240ea249a25ab162c77b` | v1.6 notes | Absent | `crates/fs`, workspace dependency | `adapt` | #58994 | Task 2.1 watch/unwatch hang regression | `proposed` | — | Use the final compatible `notify` revision. |
| P0 | [#59714](https://github.com/zed-industries/zed/pull/59714) | `37679b98a558a0c8a46b46761b677494fdbfb011` | v1.8 notes | Absent | `crates/fs` | `adapt` | #58994, #59045 | Task 2.1 case-only and canonical-path events | `proposed` | — | Case-insensitive filesystem correctness. |
| P1 | [#59560](https://github.com/zed-industries/zed/pull/59560) | `3723eef7f673300cc6818ae7a327fc6a30952068` | v1.8 notes | Absent | `crates/fs` | `adapt` | #58994, #59045 | Task 2.1 burst-event responsiveness | `proposed` | — | Dispatch watcher events without reader-thread thrashing. |
| P0 | [#58867](https://github.com/zed-industries/zed/pull/58867) | `7f6f93c089e5ed50342e2c4288a71545ddaf4f5d` | v1.7 notes | Absent | `crates/lsp`, `crates/project` | `candidate` | None | Task 2.2 bounded queue and blocked-sender shutdown | `proposed` | — | No Flint overlap was found in the initial file comparison. |
| P1 | [#61176](https://github.com/zed-industries/zed/pull/61176) | `c7148c8190d7740d51f6e380f993ba589aaf0751` | v1.12 notes | Absent | Removed native agent crates; inspect `crates/agent_threads` | `audit` | #58867 | Task 2.3 source and reachability evidence | `proposed` | — | Expected exclusion because native `agent` and `agent_ui` were removed. |
| P0 | [#58683](https://github.com/zed-industries/zed/pull/58683) | `7854e4535ddb8fee8f0d72b46bbf98c28a1f4463` | v1.10 notes | Absent | `crates/terminal` | `adapt` | None | Task 2.4 Linux PTY descriptor stress test | `proposed` | — | Linux-specific. |
| P0 | [#59128](https://github.com/zed-industries/zed/pull/59128) | `e1bfcf85db56f75a2f6d67143aad2da22c3d2240` | v1.8 notes | Absent | `crates/gpui_macos`, `crates/util` | `adapt` | Resolve #59156 and #59358 | Task 2.5 failed-spawn descriptor test | `proposed` | — | Port the final ownership model, not an intermediate patch. |
| P0 | [#59156](https://github.com/zed-industries/zed/pull/59156) | `d4cc8d240965e1b3c86b1132df2278e4d01333f6` | v1.8 notes | Absent | workspace async-process patch, `crates/gpui` | `adapt` | Review #59358 | Task 2.5 repeated child-exit reaping | `proposed` | — | Temporary patch may be superseded by #59358. |
| P0 | [#59358](https://github.com/zed-industries/zed/pull/59358) | `a873cf402c8d5ffa13ab54efd29ccd4df59c7e46` | later main correction | Absent | `crates/gpui`, workspace dependencies | `audit` | #59128, #59156 | Task 2.5 final process ownership behavior | `proposed` | — | Removes GPUI's async-process dependency upstream. |
| P0 | [#58885](https://github.com/zed-industries/zed/pull/58885) | `c642b422deaf6119aad2943ea22ec3074f39ef3c` | v1.8 notes | Absent | `crates/util`, Windows process launchers | `adapt` | None | Task 2.6 child and grandchild termination | `proposed` | — | Windows-specific Job Object behavior. |

## Wave 3: Remote development and terminal reliability

| Priority | Upstream PR | Final commit | Provenance | Fork ancestry | Flint paths | Strategy | Prerequisites | Required tests | Status | Flint PR | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| P1 | [#58157](https://github.com/zed-industries/zed/pull/58157) | `36a3a2a784fb51f7e58bc6f90e9e0202587ad4bb` | v1.8 notes | Absent | `crates/proto`, `crates/project`, `crates/remote_server` | `adapt` | None | Task 3.1 protocol round-trip and remote editing events | `proposed` | — | Preserve changed paths end to end. |
| P1 | [#59272](https://github.com/zed-industries/zed/pull/59272) | `338530f2e9b36ea05cc27db96e8f2270bd32cd12` | v1.8 notes | Absent | `crates/recent_projects`, `crates/workspace` | `adapt` | #58157 | Task 3.2 local/remote project transition | `proposed` | — | Pair with #53953 identity semantics. |
| P1 | [#53953](https://github.com/zed-industries/zed/pull/53953) | `2838ea3f59458fc550d844e78fb4fec8eaf39fa3` | v1.12 notes | Absent | `crates/recent_projects` | `adapt` | #58157 | Task 3.2 identical display path, distinct route | `proposed` | — | Preserve both local and remote recent entries. |
